...
export class MarkdownComponentDataSource {
    ...
    async getComponents(): Promise<Map<string, Map<string, Component>>> {
        const sharedUrl = `https://api.github.com/repos/grafana/alloy/contents/docs/sources/shared/reference/components?ref=${this.version}`
        const componentsUrl = `https://api.github.com/repos/grafana/alloy/contents/docs/sources/reference/components?ref=${this.version}`
        const configBlocksUrl = `https://api.github.com/repos/grafana/alloy/contents/docs/sources/reference/config-blocks?ref=${this.version}`
        ...
        const componentFolders = await this.fetchFolders(componentsUrl)
        for (const folder of componentFolders) {
            if (folder.files) {
                let m = new Map<string, Component>()
                for (const file of folder.files) {
                    try {
                        const fileResponse = await axios.get(file.download_url)
                        let component = this.parseComponentFile(file.name, fileResponse.data)
                        m.set(component.name, component)
                    } catch (error) {
                        console.error(`Error fetching content for ${file.name}:`, error)
                    }
                }
                components.set(folder.name, m)
            }
        }
    ...
    async fetchFolders(url: string): Promise<Folder[]> {
        try {
          ...
          for (const item of items) {
            if (item.type === 'dir') {
              const folder: Folder = { ...item, files: [] };
              const url = `https://api.github.com/repos/grafana/alloy/contents/docs/sources/reference/components/${item.name}?ref=${this.version}`
              folder.files = await this.fetchFiles(url);
              folders.push(folder);
            }
          }
